        ...
        stage('Deploy') {
            steps {
                withCredentials([usernameColonPassword(credentialsId: '1a5f4c2f-9662-415c-9e26-58b6840b2d49', variable: 'CREDS')]) {
                    ...
                    httpRequest consoleLogResponseBody: true, contentType: 'APPLICATION_JSON', customHeaders: [[maskValue: false, name: 'Authorization', value: "Basic " + CREDS.bytes.encodeBase64().toString()]], httpMode: 'POST', ignoreSslErrors: true, responseHandle: 'NONE', timeout: 30, url: 'http://192.168.0.25/api/v2/job_templates/7/launch/', validResponseCodes: '201'
                }
            }
        }
